...
Editor = function(chromeless, themes,editable, model, graph)
{
 ...
 * Sets the XML node for the current diagram.
 */
Editor.prototype.resetGraph = function(updatedGraph)
{
...
Editor.prototype.readGraphState = function(node)
{
	...
	if (this.graph.fitToScreen) {
		let frameHeight = (window.innerHeight * 87.82)/ 100;
		let frameWidth = (window.innerWidth  * 95.60)/ 100 ;
		var scaleToTranslate = 1;
		var isGraphSizeAvailable = setInterval(()=>{
		if(this.graph.minimumGraphSize){
			clearInterval(isGraphSizeAvailable)
			const minGraphHeight = this.graph.minimumGraphSize.height;
			const minGraphWidth = this.graph.minimumGraphSize.width;
	  
			this.graph['scaleToTranslate'] = Math.min(frameHeight / minGraphHeight, frameWidth / minGraphWidth);
			this.graph.view.scaleAndTranslate(this.graph.scaleToTranslate.toFixed(2));
			this.graph.zoomOut()
		}
		},50)
	...
	if (!isNaN(pw) && !isNaN(ph))
	{
		...
		this.graph.pageFormat = new mxRectangle(0, 0, 50, 50);
	}
...
// Editor.prototype.renderGraph = function(){
// }
 ...
 */
function Dialog(editorUi, elt, w, h, modal, closable, onClose, noScroll, transparent, onResize, ignoreBgClick,newClass)
{
	...
	var classname;
	if(transparent){
		classname = 'geTransDialog';
	}else if(newClass){
		classname = 'geDialog elmDialog';
	}else{
		classname = 'geDialog'
	}
	...
	var div = editorUi.createDiv(classname);

	// if(newClass){
	// 	div.classList.add('elmDialog')
	// }
...
(function()
{
	...
	mxGraphView.prototype.validateBackgroundStyles = function()
	{
		...
		if (graph.view.backgroundPageShape != null) {
			...
			// graph.view.backgroundPageShape.node.style.position = "absolute";
			// graph.view.backgroundPageShape.node.style.backgroundImage = image;
			// graph.view.backgroundPageShape.node.style.opacity = "0";
